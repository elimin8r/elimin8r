...
function elimin8r_hex_opacity( $color, $opacity ) {
...
function elimin8r_minify_css($css) {
    // Remove comments
...
function elimin8r_customizer_css() {
	...
	$css = '
		...
		body .site-main { background-color: ' . elimin8r_hex_opacity( $content_color, $content_transparency ) . '; }
		...
		body .site-header { background-color: ' . elimin8r_hex_opacity( $header_color, $header_transparency ) . '; }
		...
		body .site-footer { background-color: ' . elimin8r_hex_opacity( $footer_color, $footer_transparency ) . '; }
	...
	// Output the styles
	if ( ! empty( $css ) ) {
		echo '<style>' . elimin8r_minify_css( $css ) . '</style>' . PHP_EOL;
	}
}
add_action( 'wp_head', 'elimin8r_customizer_css' );