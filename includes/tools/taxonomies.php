...
add_action( 'admin_menu', 'elimin8r_register_taxonomy_settings_menu' );
function elimin8r_register_taxonomy_settings_menu() {
    // Check if administrator
    if ( !current_user_can( 'manage_options' ) ) {
        return;
    }

    add_management_page( 'Taxonomies', 'Taxonomies', 'manage_options', 'taxonomy_settings', 'elimin8r_taxonomy_settings_page' );
}

// Render the settings page
function elimin8r_taxonomy_settings_page() {
    // Check if administrator
    if ( !current_user_can( 'manage_options' ) ) {
        return;
    }
    ...
    <div class="wrap">
        ...
        <form method="post">
            <?php
            elimin8r_taxonomy_delete_callback();
...
add_action( 'admin_init', 'elimin8r_register_taxonomy_settings' );
function elimin8r_register_taxonomy_settings() {
    // Register the settings section
    add_settings_section( 'taxonomy_section', 'Add a new taxonomy', '', 'taxonomy_settings' );

    // Register the input fields
    add_settings_field( 'taxonomy_label', 'Taxonomy Label', 'elimin8r_taxonomy_label_callback', 'taxonomy_settings', 'taxonomy_section' );
    add_settings_field( 'taxonomy_slug', 'Taxonomy Slug', 'elimin8r_taxonomy_slug_callback', 'taxonomy_settings', 'taxonomy_section' );
    add_settings_field( 'taxonomy_post_type', 'Post Type', 'elimin8r_taxonomy_post_type_callback', 'taxonomy_settings', 'taxonomy_section' );
...
function elimin8r_taxonomy_delete_callback() {
    $taxonomies = get_option( 'elimin8r_custom_taxonomies', array() );
...
add_action('init', 'elimin8r_handle_taxonomy_registration');
function elimin8r_handle_taxonomy_registration() {
    $taxonomies = get_option( 'elimin8r_custom_taxonomies', array() );
...
add_action( 'admin_init', 'elimin8r_handle_taxonomy_form_submission' );
function elimin8r_handle_taxonomy_form_submission() {
    if ( isset( $_POST['taxonomy_label'] ) && isset( $_POST['taxonomy_slug'] ) && isset( $_POST['taxonomy_post_type'] ) && !empty( $_POST['taxonomy_label'] ) && !empty( $_POST['taxonomy_slug'] ) && !empty( $_POST['taxonomy_post_type'] ) ) {
        ...
        update_option( 'elimin8r_custom_taxonomies', $taxonomies );
